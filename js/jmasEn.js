let audios = []
let filePrefix = "https://archive.org/download/compendiumCorrectCreed/";


function goToPageFromSpinbox(event) {
    const pageNum = event.target.value;
    if (pageNum) {
        window.location.href = `/compendiumCorrectCreed/lesson${pageNum}/`;
    }
}

document.getElementById('first').addEventListener('click', () => {
    window.location.href = `/compendiumCorrectCreed/lesson1/`;
});

document.getElementById('prev').addEventListener('click', () => {
    const current = parseInt(document.getElementById('pagination-spinbox').value);
    if (current > 1) window.location.href = `/compendiumCorrectCreed/lesson${current - 1}/`;
});

document.getElementById('next').addEventListener('click', () => {
    const current = parseInt(document.getElementById('pagination-spinbox').value);
    if(audios.length > 0 && current + 1 < audios.length)
    window.location.href = `/compendiumCorrectCreed/lesson${current + 1}/`;
});

document.getElementById('last').addEventListener('click', () => {
    if (audios.length > 0)
    {
        window.location.href = `/compendiumCorrectCreed/lesson${audios.length}/`;
    }
});

const paginationSpinbox = document.getElementById('pagination-spinbox');

fetch(`/json/jmasLessonsEn.json?ts=${Date.now()}`)
